    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-wrap items-center justify-between px-2 gap-4">
        ...
        <div className="flex flex-wrap gap-3">
          {Object.entries(totalByCurrency).map(([curr, amt]) => (
            <p key={curr} className="text-sm font-bold text-slate-900 dark:text-white">
              {curr} Total: <span className="text-red-500 dark:text-red-400">{fmt(amt, curr)}</span>
            </p>
          ))}
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300">
        {loading ? (
          ...
          <>
            <div className="hidden sm:grid grid-cols-[1fr_100px_120px_120px_96px] gap-4 px-6 py-4 border-b border-slate-100 dark:border-slate-800 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide bg-gradient-to-r from-slate-50 to-transparent dark:from-slate-800/50">
              <span>Title</span>
              <span>Category</span>
              <span>Date</span>
            ...
            <ul className="divide-y divide-slate-100 dark:divide-slate-800">
              {filtered.map((e, idx) => (
                <li key={e.id} className="flex sm:grid sm:grid-cols-[1fr_100px_120px_120px_96px] gap-4 items-center px-6 py-4 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors duration-200 group animate-in fade-in duration-300" style={{ animationDelay: `${idx * 50}ms` }}>
                  ...
                  <div className="hidden sm:flex flex-col items-end">
                    <span className="text-sm font-bold text-red-500 dark:text-red-400 text-right">{fmt(e.amount, e.currency)}</span>
                  ...
                  <div className="sm:hidden ml-auto flex items-center gap-2">
                    <span className="text-sm font-bold text-red-500">{fmt(e.amount, e.currency)}</span>
