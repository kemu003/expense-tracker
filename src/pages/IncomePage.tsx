...
interface IncomePageProps {
  ...
  onUpdate: (id: string, data: Partial<Omit<Income, 'id' | 'user_id' | 'created_at'>>) => Promise<{ error: string | null }>;
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xl font-bold text-slate-900 dark:text-white mt-1">Total: <span className="text-green-600 dark:text-green-400">{fmt(total)}</span></p>
        </div>
        <button
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-lg">
        {loading ? (
          ...
          <>
            <div className="hidden sm:grid grid-cols-[1fr_120px_100px_96px] gap-4 px-6 py-4 border-b border-slate-100 dark:border-slate-800 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide bg-gradient-to-r from-slate-50 to-transparent dark:from-slate-800/50">
              <span>Source</span>
              <span>Date</span>
            ...
            <ul className="divide-y divide-slate-100 dark:divide-slate-800">
              {income.map((e, idx) => (
                <li key={e.id} className="flex sm:grid sm:grid-cols-[1fr_120px_100px_96px] gap-4 items-center px-6 py-4 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors duration-200 group animate-in fade-in duration-300" style={{ animationDelay: `${idx * 50}ms` }}>
                  ...
                  <span className="hidden sm:block text-sm font-bold text-green-600 dark:text-green-400 text-right">{fmt(e.amount)}</span>
                  ...
                  <div className="sm:hidden ml-auto flex items-center gap-2">
                    <span className="text-sm font-bold text-green-600">{fmt(e.amount)}</span>
